...
from urllib.request import urlopen, build_opener, HTTPCookieProcessor, HTTPRedirectHandler
...
def auth(*,email, password, client_id, scope):
    ...
    def auth_user(email, password, client_id, scope, opener):
        ...
        if parser.method == "POST":
            response = opener.open(parser.url, urlencode(parser.params).encode('ascii'))
        else:
...
def __link_params(params):
    params_list = []
    for key in params:
        params_list.append('%s=%s'%(key, params[key]))
    res = '&'.join(params_list)
    return res[1:]

def method_request(*, token=None, method_name=None, params={}):
    print('request method')
    if not isinstance(token, str) and not isinstance(method_name, str):
        raise AttributeError('Arguments must be string')
    url = 'https://api.vk.com/method/'+method_name+'?'+__link_params(params)+'&access_token='+token
    print(url)
    request = urlopen(url)
